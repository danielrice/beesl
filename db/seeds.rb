...
[ 'Education', 'Research', 'Faculty & Members', 'Advisory Board', 'Financial Assistance', 'Program Sponsors', 'Services'  ].each do |section_name|
	Section.find_or_create_by_name(section_name, :has_sidebar => true)
end
...
=end
	...
	Section.all.each { |s|
		2.times do Factory.create(:page, :section => s) end
	}
	puts "---created pages"
...
=end
	Page.all.each { |p|
    3.times do Factory.create(:panel, :page => p) end
  }
  puts "---created panels"
else
